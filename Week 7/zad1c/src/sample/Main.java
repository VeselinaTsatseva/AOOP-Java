...
public class Main extends Application {
    ...
    public void drawRectangleDiagonalLines(Rectangle rectangle, Group root){
        double x = rectangle.getUPoint().getCoordinates()[0];
        double y = rectangle.getUPoint().getCoordinates()[1];
        double width = rectangle.getWidth();
        double height = rectangle.getHeight();

        Point firstPoint = rectangle.getUPoint();
        Point secondPoint = new Point(new double[]{x + width, y});
        Point thirdPoint = new Point(new double[]{x + width, y + height});
        Point fourthPoint = new Point(new double[]{x, y + height});
